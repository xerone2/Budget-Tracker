...
def display_tasks(tasks):
    ...
    reset = "\033[0m"
    table = PrettyTable()
    table.field_names = [f"{bold}Task No.{reset}", f"{bold}Title{reset}", f"{bold}Priority{reset}", f"{bold}Created on{reset}", f"{bold}Due on{reset}", f"{bold}Completed{reset}"]

    for index, task in enumerate(tasks, start=1):
        completed = "Yes" if task.get("completed") else "No"
        table.add_row([index, task["title"], task["priority"], task["date_assigned"], task["due_date"], completed])
...
def add_task(tasks):
    ...
    new_task = {"title": title, "priority": priority, "date_assigned": date_assigned, "due_date": due_date}
